...
(function () {
  ...
  function create_fragment$1(ctx) {
  	...
  	let mounted;
  	let dispose;
  	let if_block = /*$check*/ ctx[0] == true && create_if_block$1(ctx);
  	...
  	const block = {
  		c: function create() {
  			...
  			add_location(figure, file$1, 8, 2, 191);
  			add_location(h1, file$1, 10, 3, 289);
  			add_location(blockquote, file$1, 12, 4, 313);
  			add_location(h3, file$1, 11, 3, 303);
  			attr_dev(figcaption, "class", "js-symbol js-symbol_desc");
  			add_location(figcaption, file$1, 9, 2, 239);
  			attr_dev(main, "class", "c-main");
  			add_location(main, file$1, 7, 1, 162);
  			attr_dev(dialog, "id", "logger");
  			attr_dev(dialog, "class", "js-log c-log");
  			add_location(dialog, file$1, 17, 2, 390);
  			add_location(footer, file$1, 16, 1, 378);
  		},
  		l: function claim(nodes) {
  			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
  		},
  		m: function mount(target, anchor) {
  			...
  			if (!mounted) {
  				dispose = listen_dev(main, "", /*_handler*/ ctx[1], false, false, false);
  				mounted = true;
  			}
  		},
  		p: function update(ctx, [dirty]) {
  ...
  function instance$1($$self, $$props, $$invalidate) {
  	...
  	component_subscribe($$self, check$1, $$value => $$invalidate(0, $check = $$value));
  	const writable_props = [];
  	...
  	$$self.$capture_state = () => ({ ip: ip$1, check: check$1, sc: sc$1, Modal, $check });
  	return [$check, _handler];
  }
