...
(function () {
  ...
  const ws = derived(port, x => new WebSocket("ws://" + ip + "/" + (x)));
  ...
  let port$1 = writable('');
  const unsubscribe$1 = port$1.subscribe(val => {
  });
  const ws$1 = derived(port$1, x => new WebSocket("ws://" + ip$1 + "/" + (x)));
  ...
  function create_if_block(ctx) {
  	...
  	const block = {
  		...
  		m: function mount(target, anchor) {
  			...
  			set_input_value(input, /*data*/ ctx[1].room);
  		...
  		p: function update(ctx, dirty) {
  			...
  			if (dirty & /*data*/ 2 && input.value !== /*data*/ ctx[1].room) {
  				set_input_value(input, /*data*/ ctx[1].room);
  			}
  ...
  function create_if_block_1(ctx) {
  	...
  	const block = {
  		c: function create() {
  			...
  			add_location(label, file, 45, 3, 992);
  			attr_dev(input, "type", "");
  			attr_dev(input, "name", "");
  			add_location(input, file, 45, 29, 1018);
  		},
  		m: function mount(target, anchor) {
  ...
  function instance($$self, $$props, $$invalidate) {
  	let $ws;
  	validate_store(ws$1, "ws");
  	component_subscribe($$self, ws$1, $$value => $$invalidate(5, $ws = $$value));
  	let { check = false } = $$props;
  	let data = { room: "", name: "" };

  	function sendData(x) {
  		console.log($ws);
  		port$1.set(data.room);
  		$ws.send(JSON.stringify(data));
  		console.log(get_store_value(ws$1));
  	}
